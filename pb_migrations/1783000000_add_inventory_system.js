...
migrate((app) => {
  ...
  if (products) {
    ...
    products.fields.add(
      new NumberField({ name: "stockQty", required: false, onlyInt: true, min: 0 }),
      new NumberField({ name: "lowStockThreshold", required: false, onlyInt: true, min: 0 }),
      new NumberField({ name: "damagedQty", required: false, onlyInt: true, min: 0 }),
    )
    app.save(products)
  }
