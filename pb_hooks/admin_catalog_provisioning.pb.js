...
onRecordAfterCreateSuccess((e) => {
  ...
  try {
    ...
    // The admin record has already been created successfully. Catalog
    // provisioning is supplemental and must not turn the create response
    // into an error when a product/schema issue occurs.
    console.error(
      `[admin_catalog_provisioning] Failed to provision catalog for admin ${e.record.id}:`,
      error,
    );
  }
  e.next();
